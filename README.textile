...
h2. Example Usage
Batch size is the number of database updates to wait for before initializing the
replication process. A database updates may not necessarily mean a document has 
been updated. The script will also start the replication process every X seconds
to make sure that the target database is updated every so often.
