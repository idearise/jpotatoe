...
during its periodic time check even if there have been no other database updates.
* A CouchDB "database update" message does not necessarily mean a document has 
been updated...See the previous point about CouchDB sending a database update 
...
* The script doesn't handle specific batch sizes or times for different 
databases.
